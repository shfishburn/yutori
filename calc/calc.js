(function () {
  ...
  function simulateDynamicMetabolicAdaptation(params) {
    try {
      ...
      const safeMinBF = gender === "male" ? 10 : 15; // Set the safe minimum body fat percentage based on gender
      const effectiveTargetBF = Math.max(userTargetBF, safeMinBF); // Ensure target BF isn't below the safe minimum
      
      let currentWeight = isKg ? initialWeight * 2.20462 : initialWeight; // Convert weight to lbs if needed
      let currentFat = currentWeight * (bodyFatPct / 100); // Calculate fat mass based on body fat percentage
      let currentLean = currentWeight - currentFat; // Calculate lean mass
      let currentBF = bodyFatPct; // Current body fat percentage
      
      const weightKg = isKg ? initialWeight : currentWeight / 2.20462; // Convert weight to kg if needed
      const baselineRMR =
        gender === "male"
          ? 10 * weightKg + 6.25 * heightCm - 5 * age + 5
          : 10 * weightKg + 6.25 * heightCm - 5 * age - 161; // Calculate Basal Metabolic Rate (RMR)
      const baselineTDEE = baselineRMR * activityMultiplier; // Calculate Total Daily Energy Expenditure (TDEE)
      
      const dailyDeficit = Number(deficitValue); // Convert deficit value to a number
      const weeklyDeficit = Math.abs(dailyDeficit) * 7; // Calculate total weekly calorie deficit
      const beta = 0.10; // Define metabolic adaptation coefficient
      const E_eff = 4000; // Energy efficiency factor
      
      // Function to partition fat ratio based on body fat percentage
      function partitionFatRatio(bf) {
        if (bf > 30) return 0.90; // High fat percentage leads to higher fat loss rate
        else if (bf >= 20 && bf <= 30) return 0.75; // Moderate fat percentage
        else return 0.60; // Lower fat percentage, lower fat loss rate
      }
  ...
  function populateGoalBodyFatOptions(gender) {
    ...
    // Always clear and repopulate to be defensive
    ...
    // Restore selection if available; otherwise, set to default "fitness"
  ...
  class ChartManager {
    ...
    async renderForecastCharts(weeklyData, startDate) {
      // Ensure the canvases exist; if not, create them
      const step3 = document.getElementById("step3");
      const requiredCharts = [
        { id: "calorieChart", label: "Calorie Forecast" },
        { id: "macroChart", label: "Macro Trajectory" },
        { id: "weightChart", label: "Weight Loss Trajectory" },
      ];
      requiredCharts.forEach((chart) => {
        if (!document.getElementById(chart.id)) {
          ...
          canvas.setAttribute("aria-label", `${chart.label} chart`);
  ...
  function doFinalCalculation() {
    try {
      ...
      // Do not repopulate the fat goal options here so as to preserve the user's selection.
  ...
  // Updated back button handler: if leaving step3, destroy charts so recalculation will re-render them.
